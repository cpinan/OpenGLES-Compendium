...
class TouchCubeGLSurfaceView(
    context: Context
) : BaseGLSurfaceView(context) {

    private var previousTouchX = 0F
    private var previousTouchY = 0F
    private lateinit var customRenderer: TouchCubeGLRenderer

    override fun setupRenderer(): BaseGLRenderer {
        return TouchCubeGLRenderer(context)
    }

    override fun additionalSetup() {
        super.additionalSetup()
        renderMode = RENDERMODE_WHEN_DIRTY
        customRenderer = renderer as TouchCubeGLRenderer
    }
